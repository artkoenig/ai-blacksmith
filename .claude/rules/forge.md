...
- `plugins/forge/` is the source. `.claude/skills`, `.claude/agents` and `.claude/workflows` symlink
  into it, so edits are live in the session. Never edit through the symlink path.
